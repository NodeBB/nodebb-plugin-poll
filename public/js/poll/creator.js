...
(function(Poll) {
	...
	function initComposer() {
		require([
			...
			'composer/formatting',
			'composer/controls'
		], function(composer, formatting, controls) {
			...
			return {
				init: function() {
					...
					require(['translator'], function(translator) {
						translator.translate('[[poll:creator_title]]', function(
							translated
						) {
	...
	function composerBtnHandle(composer, textarea) {
		...
		Poll.sockets.canCreate({ cid: post.cid, pid: post.pid }, function(
			err,
			canCreate
		) {
			if (err || !canCreate) {
			...
			Poll.sockets.getConfig(null, function(err, config) {
				...
				Creator.show(poll, config, function(data) {
					...
					textarea.value = Poll.serializer.removeMarkup(
						textarea.value
					);
