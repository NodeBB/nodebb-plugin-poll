			...
			<div class="table-responsive">
				<table id="poll-table" class="table">
					<thead>
						<tr class="text-sm">
							<th>ID</th>
							<th class="text-nowrap">{{tx("poll:poll_title")}}</th>
							<th class="text-nowrap">{{tx("poll:created-time")}}</th>
							<th class="text-nowrap">{{tx("poll:end-time")}}</th>
							<th class="text-end text-nowrap text-capitalize">{{tx("poll:votes")}}</th>
						</tr>
					</thead>
		...
		<div id="spy-container" class="col-12 col-lg-3" tabindex="0">
			<form class="form poll-settings">
				<div class="form-check mb-3">
					<label class="form-check-label" for="allowGuestsToViewResults">{{tx("poll:allow_guests")}}</label>
					<input class="form-check-input" type="checkbox" id="allowGuestsToViewResults" name="allowGuestsToViewResults">
				</div>

				<div class="mb-3">
					<label class="form-label" for="maxOptions">{{tx("poll:max_options")}}</label>
					<input type="number" class="form-control" id="maxOptions" placeholder="10" min="1" max="100" name="maxOptions">
				</div>

				<div class="mb-3">
					<label class="form-label" for="defaultTitle">{{tx("poll:default_title")}}</label>
					<input type="text" class="form-control" id="defaultTitle" placeholder="Poll" name="defaultTitle">
				</div>
				<div>
					<label class="form-label" for="maximumVotesPerUser">{{tx("poll:max_votes")}}</label>
