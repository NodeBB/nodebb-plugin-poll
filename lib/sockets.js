...
Sockets.vote = async function (socket, data) {
	...
	if (parseInt(pollData.remote, 10) === 1) {
		await federateVoteChange({
			pollData, socket, voteAnon: data.voteAnon, removedOptionIds: [], addedOptionIds: data.options,
		});
	}
...
Sockets.updateVote = async function (socket, data) {
	...
	// Capture the prior vote + anon flag so we can federate the diff (remote polls only)
	...
	if (isRemote) {
		...
		await federateVoteChange({ pollData, socket, voteAnon, removedOptionIds, addedOptionIds });
	}
...
// Build a FEP-9967 vote Note. Stable, idempotent id so re-sends and
// Deletes dedupe remotely.
...
async function federateVoteChange({ pollData, socket, voteAnon, removedOptionIds, addedOptionIds }) {
	...
	// Delete removed options (the vote Note is an entity; retraction deletes it)
