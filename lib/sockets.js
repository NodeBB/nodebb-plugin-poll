...
(function(Sockets) {
	...
	Sockets.get = function(socket, data, callback) {
		...
		if (!data) {
			return callback(
				new Error('Invalid request, request data is not defined')
			);
		}
		...
		if (!socket.uid && !allowAnon) {
			return callback(
				new Error('Invalid request, anonymous access is not allowed')
			);
		}
		...
		Poll.get(pollId, socket.uid, !!socket.uid, function(err, pollData) {
			...
			pollData.optionType =
				parseInt(pollData.settings.maxvotes, 10) > 1
					? 'checkbox'
					: 'radio';
	...
	Sockets.vote = function(socket, data, callback) {
		...
		async.parallel(
			...
			function(err, result) {
				...
				if (
					data.options.length > parseInt(result.settings.maxvotes, 10)
				) {
					return callback(
						new Error(
							'You can only vote for ' +
								result.settings.maxvotes +
								' options on this poll.'
						)
					);
				}
				...
				if (err || !result.canVote || !data.options.length) {
					return callback(
						new Error('Already voted or invalid option')
					);
				}
				...
				Vote.add(data, function(err) {
					...
					Poll.get(data.pollId, socket.uid, false, function(
						err,
						pollData
					) {
						NodeBB.SocketIndex.server.sockets.emit(
							'event:poll.voteChange',
							pollData
						);
	...
	Sockets.updateVote = function(socket, data, callback) {
		if (!socket.uid) {
			return callback(
				new Error('You need to be logged in to make changes')
			);
		}
		if (
			!data ||
		...
		async.parallel(
			...
			function(err, result) {
				...
				Vote.update(data, function(err) {
					if (err) {
						return callback(
							new Error('Error during updating vote')
						);
					}
		...
		async.parallel(
			...
			function(err, result) {
				...
				Vote.remove(data, function(err) {
					if (err) {
						return callback(
							new Error('Error during removing vote')
						);
					}
					Poll.get(data.pollId, socket.uid, false, function(
						err,
	...
	Sockets.getOptionDetails = function(socket, data, callback) {
		if (
			...
			!data ||
			isNaN(parseInt(data.pollId, 10)) ||
			isNaN(parseInt(data.optionId, 10))
		) {
			return callback(new Error('Invalid request'));
		}
			...
			async.parallel(
				...
				function(err, usersPreferences) {
					...
					result.votes = users.map(function(user) {
						var isAnonymous = preferences.find(function(
							preference
						) {
							if (!preference) {
								return false;
	...
	Sockets.canCreate = function(socket, data, callback) {
		if (
			!socket.uid ||
			!data ||
			(isNaN(parseInt(data.cid, 10)) && isNaN(parseInt(data.pid, 10)))
		) {
			return callback(new Error('Invalid request'));
		}
	...
	function checkPrivs(cid, socketUid, callback) {
		NodeBB.Privileges.categories.can(
			'poll:create',
			cid,
			socketUid,
			function(err, can) {
				if (err || !can) {
					return callback(
						new Error('[[poll:error.privilege.create]]'),
						can
					);
				} else {
