...
(function(Scheduler) {
	...
	Scheduler.add = function(pollId) {
		...
		Poll.getSettings(pollId, function(err, settings) {
			...
			if (!settings) {
				return NodeBB.winston.error(
					'[nodebb-plugin-poll/scheduler] Poll ID ' +
						pollId +
						' has no settings!'
				);
			}
			...
			if (end < now) {
				...
				NodeBB.winston.verbose(
					'[nodebb-plugin-poll/scheduler] Starting scheduler for poll with ID ' +
						pollId +
						' to end on ' +
						date
				);
	...
	Scheduler.end = function(pollId) {
		NodeBB.winston.verbose(
			'[nodebb-plugin-poll/scheduler] Ending poll with ID ' + pollId
		);
