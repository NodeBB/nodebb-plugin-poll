...
(function(Hooks) {
	...
	async function savePoll(obj) {
		...
		// Hack to handle quillDelta in composer-quill
		if (postobj.quillDelta) {
			postobj.quillDelta = JSON.parse(postobj.quillDelta);
			postobj.quillDelta.ops = postobj.quillDelta.ops.map((set) => {
				if (!set.insert) {
					return set;
				}

				set.insert = Serializer.removeMarkup(set.insert);
				return set;
			});
			postobj.quillDelta = JSON.stringify(postobj.quillDelta);
		}
