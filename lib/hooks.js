...
Hooks.filter.activitypubMocksNote = async function (hookData) {
	...
	// Embed the poll title in the content so it renders as the question text
	// on Mastodon (which displays `content` above the options).
	if (info.title) {
		object.content = `<p>${info.title}</p>${object.content || ''}`;
	}
	if (isMulti) {
...
Hooks.filter.activitypubCreate = async function (hookData) {
	...
	// Publish Update(Question) to all recipients (public, followers, voters)
	setImmediate(async () => {
		try {
			const utils = nodebb.require('./src/utils');
			const authorUid = parseInt(await posts.getPostField(pid, 'uid'), 10);
			const remoteVoters = (await db.getSortedSetMembers(`poll:${pollId}:voters`))
				.filter(v => !utils.isNumber(v));

			const { to, cc, targets } = await activitypub.buildRecipients(
				{ to: ['https://www.w3.org/ns/activitystreams#Public'], cc: remoteVoters },
				{ pid, uid: authorUid }
			);

			questionObject.to = to;
			questionObject.cc = cc;

			await activitypub.send('uid', authorUid, Array.from(targets), {
				id: `${object.id || object.inReplyTo}#update/${Date.now()}`,
				type: 'Update',
				actor: questionObject.attributedTo,
				to,
				cc,
