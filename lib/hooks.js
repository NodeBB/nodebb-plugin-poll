...
Hooks.filter.postCreate = async function (hookData) {
	...
	if (Array.isArray(data?.polls) && data.polls.length) {
		await checkMaxOptions(data.polls);
...
Hooks.filter.postEdit = async function (hookData) {
	...
	await checkMaxOptions(pollData);
...
async function checkMaxOptions(polls) {
	if (!Array.isArray(polls) || !polls.length) {
		return;
	}
	const { maxOptions } = await Config.getSettings();
	polls.forEach((poll) => {
		if (poll && poll.options.length > maxOptions) {
