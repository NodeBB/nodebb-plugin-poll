...
Vote.canUpdateVote = async function (uid, pollId) {
	const pollInfo = await Poll.getInfo(pollId, false);
	if (!pollInfo) {
		throw new Error('[[error:invalid-data]]');
	}
	if (parseInt(pollInfo.remote, 10) === 1) {
		// Federated (remote) polls are not votable locally (Phase 1).
		return false;
	}
	const [canRead, ended, deleted, updateDisabled] = await Promise.all([
		privileges.posts.can('topics:read', pollInfo.pid, uid),
		Poll.hasEnded(pollId),
		Poll.isDeleted(pollId),
		Poll.doesDisallowVoteUpdate(pollId),
	]);
	return canRead && !ended && !deleted && !updateDisabled;
};
...
Vote.canVote = async function (uid, pollId) {
	...
	const [canRead, ended, deleted, hasVoted] = await Promise.all([
		privileges.posts.can('topics:read', pollInfo.pid, uid),
		Poll.hasEnded(pollId),
		Poll.isDeleted(pollId),
		Vote.hasUidVoted(uid, pollId),
	]);
	return canRead && !ended && !deleted && !hasVoted;
};
