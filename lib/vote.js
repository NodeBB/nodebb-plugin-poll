...
(function(Vote) {
	...
	Vote.add = function(voteData, callback) {
		...
		async.parallel([
			...
			function(next) {
				NodeBB.db.setObject('poll:' +  pollId + ':preferences', preferences, next)
			},
			function(next) {
				NodeBB.db.setObjectField('poll:' +  pollId + ':preferences', 'value', String(uid), next);
			},
		], function(err) {
			if (err) {
	...
	Vote.removeUidVote = function(uid, pollId, callback) {
		Vote.getUidVote(uid, pollId, function (err, vote) {
			async.parallel([
				function(next) {
					async.each(vote.options || [], function(option, next) {
