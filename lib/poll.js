...
Poll.add = async function (postData, polls) {
	...
	await validateMaxOptions(polls);
	const savePolls = [];
...
// ---------------------------------------------------------------------------
// Federated (remote) polls — FEP-9967 `Question` objects
// ---------------------------------------------------------------------------

// Stable option id derived from the option name (the FEP guarantees name
// uniqueness within a poll), so ids survive `Update`s even if order changes.
function hashOptionId(name) {
	return crypto.createHash('sha256').update(String(name)).digest('hex').slice(0, 12);
}

// Parse a Question's `_activitypub` poll data into the plugin's option shape.
function parseQuestion(ap) {
	const rawOptions = Array.isArray(ap.anyOf) ? ap.anyOf : (Array.isArray(ap.oneOf) ? ap.oneOf : []);
	const isMulti = Array.isArray(ap.anyOf);
	const options = [];
	const remoteVotes = {};
	rawOptions.forEach((opt) => {
		...
		const id = hashOptionId(opt.name);
		options.push({ id, title: opt.name });
		remoteVotes[id] = (opt.replies && opt.replies.totalItems) || 0;
	});
	return {
		options,
		remoteVotes,
		maximumVotesPerUser: isMulti ? Math.max(options.length, 1) : 1,
	};
}

function tryParseRemoteVotes(json) {
	if (!json) {
		return {};
	}
	try {
...
// Create a poll from a federated Question. Bypasses the local `maxOptions`
// limit (that cap applies to locally-authored polls, not ingested ones).
Poll.createFromQuestion = async function (postData, ap) {
	const { options, remoteVotes, maximumVotesPerUser } = parseQuestion(ap);
	const pollId = await db.incrObjectField('global', 'nextPollId');
	const poll = {
		pollId,
		title: 'Poll',
		...
		pid: postData.pid,
		deleted: 0,
		end: ap.endTime ? new Date(ap.endTime).getTime() : 0,
		maximumVotesPerUser,
		timestamp: postData.edited || postData.timestamp,
		remote: 1,
		remoteVotes: JSON.stringify(remoteVotes),
		options: JSON.stringify(options),
		...(ap.votersCount !== undefined && { remoteVotersCount: ap.votersCount }),
	};
	await Promise.all([
		db.setObject(`poll:${pollId}`, poll),
		db.sortedSetAdd('polls:createtime', poll.timestamp, pollId),
	]);
	return poll;
};

// Refresh an existing remote poll from an updated Question (FEP-9967 `Update`).
Poll.updateFromQuestion = async function (pollId, ap) {
	const { options, remoteVotes, maximumVotesPerUser } = parseQuestion(ap);
...
Poll.isRemote = async function (pollId) {
	const result = await Poll.getField(pollId, 'remote');
...
Poll.edit = async function (postData, newPolls) {
	...
	await validateMaxOptions(toAdd);
	await validateMaxOptionsForEdit(toUpdate, currentPolls);
...
Poll.get = async function (pollId, uid, withVotes = false) {
	...
	const canRead = await privileges.posts.can('topics:read', info.pid, uid);
	if (!canRead) {
...
Poll.getPollIdsByPid = async function (pid) {
	const pollIdsJson = await db.getObjectField(`post:${pid}`, 'pollIds');
	if (!pollIdsJson) {
		return [];
	}
	try {
...
Poll.getPollIdsByPids = async function (pids) {
	const postData = await db.getObjectsFields(pids.map(pid => `post:${pid}`), ['pollIds']);
...
Poll.getInfo = async function (pollId, withVotes = false) {
	...
	// Federated (remote) polls: displayed count = ingested baseline + local votes.
	// (v1 reconciliation: a local voter sees their vote reflect immediately; a small
	// over-count is accepted until the next remote Update refreshes the baseline.)
	if (parseInt(poll.remote, 10) === 1) {
		const remoteVotes = tryParseRemoteVotes(poll.remoteVotes);
		poll.options.forEach((option) => {
			if (option) {
				const baseline = remoteVotes[option.id] || 0;
				option.voteCount = baseline + (option.voteCount || 0);
			}
		});
		poll.voteCount = poll.options.reduce((sum, o) => sum + (o ? (o.voteCount || 0) : 0), 0);
	}
	return poll;
};
...
async function validateMaxOptions(polls) {
	if (!Array.isArray(polls) || !polls.length) {
		return;
	}
	const settings = await Config.getSettings();
	const { maxOptions } = settings;
	polls.forEach((poll) => {
		if (!poll) {
			return;
		}
		...
		if (optionCount > maxOptions) {
			throw new Error(`[[poll:error.max_options, ${maxOptions}]]`);
		}
	});
}

async function validateMaxOptionsForEdit(updatedPolls, currentPolls) {
	if (!Array.isArray(updatedPolls) || !updatedPolls.length) {
		return;
	}
	const settings = await Config.getSettings();
	const { maxOptions } = settings;
	updatedPolls.forEach((poll) => {
		...
		const newOptionCount = Array.isArray(poll.options) ? poll.options.length : 0;
		if (newOptionCount <= maxOptions) {
			return;
		}
		const currentPoll = currentPolls.find(p => p && String(p.pollId) === String(poll.pollId));
		const currentOptions = currentPoll ? tryParseOptions(currentPoll.options) : [];
		const currentOptionCount = Array.isArray(currentOptions) ? currentOptions.length : 0;
