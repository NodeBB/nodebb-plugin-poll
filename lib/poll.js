...
(function(Poll) {
	...
	Poll.add = function(pollData, postData, callback) {
		NodeBB.db.incrObjectField('global', 'nextPollId', function(
			err,
			pollId
		) {
			if (err) {
			...
			async.parallel(
				[
					async.apply(async.each, pollData.options, function(
						option,
						next
					) {
						async.series(
		...
		async.parallel(
			...
			function(err, pollData) {
				...
				pollData.options.forEach(function(option) {
					var percentage = (
						(option.voteCount / pollData.info.voteCount) *
						100
					).toFixed(0);
	...
	Poll.getOptions = function(pollId, withVotes, callback) {
		...
		NodeBB.db.getSetMembers('poll:' + pollId + ':options', function(
			err,
			options
		) {
			async.map(
				...
				votes: function(next) {
					if (withVotes) {
						NodeBB.db.getSortedSetRange(
							'poll:' + pollId + ':options:' + option + ':votes',
							0,
							-1,
							next
						);
	...
	Poll.doesDisallowVoteUpdate = function(pollId, callback) {
		Poll.getSettingsField(pollId, 'disallowVoteUpdate', function(
			err,
			result
		) {
			callback(err, parseInt(result, 10) === 1);
