...
(function(Poll) {
	...
	Poll.getInfo = function(pollId, callback) {
		async.parallel({
			...
			voteCount: function (next) {
				NodeBB.db.sortedSetCount('poll:' + pollId + ':voters', '-inf', '+inf', next);
			}
			...
			console.log("Poll.getInfo", results.poll);
	...
	Poll.getOption = function(pollId, option, withVotes, callback) {
		async.parallel({
			...
			voteCount: function(next) {
				NodeBB.db.sortedSetCount('poll:' + pollId + ':options:' + option + ':votes', '-inf', '+inf', next);
			}
		}, function(err, results) {
			if (err) {
				return callback(err);
			...
			// console.log("Poll.getOption", results.option);
	...
	Poll.disallowsVoteUpdate = function(pollId, callback) {
		Poll.getSettingsField(pollId, 'disallowVoteUpdate', function(err, result) {
			callback(err, parseInt(result, 10) === 1);
		});
	};

	Poll.allowsVoteUpdate = function(pollId, callback) {
		Poll.getSettingsField(pollId, 'disallowVoteUpdate', function(err, result) {
			callback(err, parseInt(result, 10) !== 1);
		});
	};
