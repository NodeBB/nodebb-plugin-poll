...
Poll.getInfo = async function (pollId, withVotes = false) {
	...
	// Federated (remote) polls: the ingested vote counts are the source of truth
	// (we can't know remote voters' uids, so the local vote sets are empty).
	if (parseInt(poll.remote, 10) === 1) {
		const remoteVotes = tryParseRemoteVotes(poll.remoteVotes);
		poll.options.forEach((option) => {
			if (option && remoteVotes[option.id] !== undefined) {
				option.voteCount = remoteVotes[option.id];
			}
		});
