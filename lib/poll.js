...
async function deletePolls(pollIds) {
...
Poll.edit = async function (postData, newPolls) {
	...
	await deletePolls(toDelete);

	await Poll.add(postData, toAdd);
	...
	if (newPollIds.length) {
		await db.setObjectField(`post:${postData.pid}`, 'pollIds', JSON.stringify(newPollIds));
	} else {
